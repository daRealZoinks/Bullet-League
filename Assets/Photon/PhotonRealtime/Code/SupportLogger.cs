...
namespace Photon.Realtime
{
...
#endif
    {
        ...
        {
            if (client == null || client.State == ClientState.PeerCreated)
            {
                return;
            }
            ...
            if (LogTrafficStats)
            {
                Debug.Log(string.Format("{0} SupportLogger {1} Ping min/max: {2}/{3}", GetFormattedTimestamp(), client.LoadBalancingPeer.VitalStatsToString(false), pingMin, pingMax));
            }
        }
        ...
        private void LogBasics()
        {
            if (this.client != null)
            {
                List<string> buildProperties = new(10);
#if SUPPORTED_UNITY
                ...
                string appIdShort = string.IsNullOrEmpty(client.AppId) || client.AppId.Length < 8 ? client.AppId : string.Concat(client.AppId[..8], "***");
